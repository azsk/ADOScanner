...
function Test-ADO{
    Param(
		...
		[ValidateSet('AzSKPreview.ADO', 'AzSKStaging.ADO' ,'AzSK.ADO')]
		[Parameter(Mandatory = $True)]
		[string]
		$AzSKModule,
	...
	try
	{
			...
			$testsettings = [TestHelper]::ADOPostCommandStartedAction($Org,$AzSKModule,$AzSKModulePath);
			[TestHelper]::SetDevTestMode($DevTestMode)
			[TestContext] $testContext = [TestContext]::new($Org, $Pat , $PSCmdlet.MyInvocation)
			...
			[TestCase[]] $testcases = [TestHelper]::GetTestCases($TestScenariosFileName,$TestScenarioID, $Feature, $ModuleName, $TestCaseIDs, $ExcludeTestCaseIDs, $Priority, $ParallelScan, $True)
						...
						if($testcase.Enabled)
						{
							...
							if($testcase.AutomationStatus -ne "Manual")
							{
								try 
								{
									...
									$testrunner = [ADOTestRunner]::new($Org,$testcase,$testsettings,$testContext,$AzSKModule)					
	...
	{
		[CommonHelper]::Log("Test-ADO: $($_)", [MessageType]::Error)
		$_
		return [CommonHelper]::AzSKTestLogFolderPath
	}
