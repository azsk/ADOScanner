...
{    
    ...
    {
        if([Helpers]::CheckMember($this.ControlSettings.Organization,"GroupsToCheckForGuestUser"))
        {
            try {
                ...
                {   
                            ...
                            try 
                            {
                                ...
                                if([Helpers]::CheckMember($response[0],"containerDescriptor"))
                                {
                                    foreach ($obj in $response) 
                                    {
                                        ...
                                        $group = $data[1]
                                        if($scope -eq $this.OrganizationContext.OrganizationName -and ($group -in $GroupsToCheckForGuestUser) )
                                        {
                                            $formattedData += @{
                                                Group = $data[1];
                                                Scope = $data[0];
                    ...
                    {   
                        ...
                        $controlResult.AddMessage("Total guest accounts having admin roles in organization are: $($results.count) ");
    ...
    {
        if([Helpers]::CheckMember($this.ControlSettings.Organization,"GroupsToCheckForInactiveUser") -and [Helpers]::CheckMember($this.ControlSettings.Organization,"InActiveUserActivityLogsPeriodInDays"))
        {
            try {
                ...
                {
                    $inactiveUsers =  @()
                    $thresholdDate =  (Get-Date).AddDays(-$($this.ControlSettings.Organization.InActiveUserActivityLogsPeriodInDays))
                    $responseObj[0].items | ForEach-Object { 
                    ...
                    {   
                                ...
                                try 
                                {
                                    ...
                                    if([Helpers]::CheckMember($response[0],"containerDescriptor"))
                                    {
                                        foreach ($obj in $response) 
                                        {
                                            ...
                                            $scope =  $data[0] -replace '[\[\]]'
                                            $group = $data[1]
                                            if($scope -eq $this.OrganizationContext.OrganizationName -and ($group -in $GroupsToCheckForInactiveUser) )
                                            {
                        ...
                        {   
                            ...
                            $controlResult.AddMessage("Total inactive users having admin roles in organization are: $($results.count) ");
