                ...
                if ($adminGroupNames.Count -gt 0)
                {
                    ...
                    if(($adminGroups | Measure-Object).Count -gt 0)
                    {
                        ...
                        if($PCSAGroup.Count -gt 0)
                        {
                            ...
                            #USE IDENTITY ID
                            $groupMembers | ForEach-Object {$allPCSAMembers += @( [PSCustomObject] @{ name = $_.displayName; mailAddress = $_.mailAddress; id = $_.originId; groupName = "Project Collection Administrator"; descriptor=$_.descriptor; directMemberOfGroup = $_.DirectMemberOfGroup } )}
