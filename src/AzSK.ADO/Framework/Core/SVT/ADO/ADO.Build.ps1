    ...
    hidden [ControlResult] CheckDPVariableGroup([ControlResult] $controlResult)
    {
        if([Helpers]::CheckMember($this.BuildObj[0],"variableGroups"))
        {
            ...
            {   
                ...
                {
                    $controlResult.AddMessage([VerificationResult]::Passed,"Variable groups used in build definition are not accessible to all pipelines or does not contain secrets.");    
                }
            }
            catch
            {
