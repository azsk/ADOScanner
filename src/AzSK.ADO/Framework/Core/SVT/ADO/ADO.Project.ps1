...
"@;
    ...
    hidden [ControlResult] CheckBroaderGroupInheritanceSettingsForRepo ([ControlResult] $controlResult) {
        ...
        try{
            ...
            $excessivePermissions = $this.ControlSettings.Repo.RestrictedRolesForBroaderGroupsInRepo;  

            $permissionSetToken = $this.ResourceContext.ResourceDetails.id
            $namespacesApiURL = "https://dev.azure.com/{0}/_apis/securitynamespaces?api-version=6.0" -f $($this.OrganizationContext.OrganizationName)
            $securityNamespacesObj = [WebRequestHelper]::InvokeGetWebRequest($namespacesApiURL);
            $repoSecurityNamespaceId = ($securityNamespacesObj | Where-Object {$_.Name -eq "Git Repositories"}).namespaceId
            ...
            $inputbody.dataProviderContext.properties.sourcePage.url = $refererUrl
            $inputbody.dataProviderContext.properties.sourcePage.routeValues.Project = $this.ResourceContext.ResourceName;
            $inputbody.dataProviderContext.properties.permissionSetId = "$($repoSecurityNamespaceId)"
            ...
            if([Helpers]::CheckMember($responseObj[0],"dataProviders") -and ($responseObj[0].dataProviders.'ms.vss-admin-web.security-view-members-data-provider') -and ([Helpers]::CheckMember($responseObj[0].dataProviders.'ms.vss-admin-web.security-view-members-data-provider',"identities")))
            {
                ...
                $broaderGroupsList = @($responseObj[0].dataProviders.'ms.vss-admin-web.security-view-members-data-provider'.identities | Where-Object { $_.subjectKind -eq 'group' -and $restrictedBroaderGroupsForRepo -contains $_.displayName })
