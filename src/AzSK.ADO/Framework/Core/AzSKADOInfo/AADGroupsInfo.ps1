...
{    
	...
	[MessageData[]] GetAADGroupsList()
	{
        ...
        if ($this.aadGroupsList.Count -gt 0)
        {
			...
			$this.PublishCustomMessage("Total number of AAD groups found: $($this.aadGroupsList.Count)", [MessageType]::Warning);
        }
        else {
		...
		$returnMsgs += [MessageData]::new("Total number of AAD groups found: $($this.aadGroupsList.Count)");
		return $returnMsgs
	}
